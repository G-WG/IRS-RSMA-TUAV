...
%                  power( zeta_c_old(k) - rho_c_old(k), 2 ) );
            2*real( (precoder_c_old'* h_ov_k(:, k)) * ...
                (h_ov_k(:, k)'*precoder_c ) ) / zeta_c_old(k) - ...
                square_abs(h_ov_k(:, k)'*precoder_c_old) * ...
                zeta_c(k) / power(zeta_p_old(k), 2) >= rho_c(k);
...
%                  power( zeta_p_old(k) - rho_p_old(k), 2 ) );
            2*real( (precoder_matrix_k_old(:, k)'* h_ov_k(:, k)) * ...
                (h_ov_k(:, k)'*precoder_matrix_k ) )/zeta_p_old(k) - ...
                pow_abs(h_ov_k(:, k)'*precoder_matrix_k_old(:, k), 2) * ...
                zeta_p(k)/power(zeta_p_old(k), 2) >= rho_p(k) ;
